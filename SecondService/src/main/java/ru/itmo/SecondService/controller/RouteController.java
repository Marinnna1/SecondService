...
public class RouteController {
    ...
    @GetMapping("/count")
    public ResponseEntity getRoutesCount() throws CertificateException, UnrecoverableKeyException, NoSuchAlgorithmException, IOException, KeyManagementException, KeyStoreException {
        Long routesCount = routeService.getRoutesCount();
        if(routesCount == 0) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Routes not found");
        }
